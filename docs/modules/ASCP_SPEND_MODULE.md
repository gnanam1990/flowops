...
Successful actions emit `LockExecuted` or `AllowanceExecuted`; governance emits authorizer, allowlist, cap, pause, and nonce-invalidation events plus the exact `GovernanceWorkflowBound` receipt event. Every Safe-only governance function now requires `workflowId` and `workflowPayloadHash`; the hash binds chain, module, selector, current-state preconditions, and proposed values. Public state supports reconciliation of authorizer epoch, pause, allowlist code hashes, consumed nonces, pending/active caps, lifetime executed principal, and per-day executed principal.
