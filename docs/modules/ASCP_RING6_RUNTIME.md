...
allows five configured stage budgets for the response: two sequential
dependency calls and the three `BOUND`, `HSM_REQUESTED`, and `SIGNED` durable
append-plus-fsync transitions. Each dependency call retains the configured
one-to-ten-second bound. It refuses any startup
