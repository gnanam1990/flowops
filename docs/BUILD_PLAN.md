...
6. x402 V2 Base Sepolia adapter and Builder Code conformance fixture.
   **Fail-closed quote/extension handling and the private-key-free hosted
   facilitator experiment harness are implemented. Protocol-aware facilitator
   settlement registration now binds an issued authorization to a canonical
   transaction candidate without trusting the facilitator as payment evidence;
   finality moves durable reservations to spent or reverted accounting. The
...
9. Escrow contracts after the dispute-state redesign. **Local implementation,
   ...
   remain gated.**
10. Dashboard and operator workflows. **Fail-closed public reads, exact-member step-up command binding, exact-digest legacy and ASCP approval/denial, durable browser recovery, persistent organization-wide authorization pause, PostgreSQL-backed active-policy capacity/reservations/payment activity, tenant-scoped reconciliation exceptions/progress, and asset-bound operational-ledger aggregates are implemented. Runtime demo records and the illustrative `/demo` route are removed. The production step-up issuer remains an integration gate; wallet balances are deliberately not inferred.**
11. Authenticated control-plane API and PostgreSQL command boundary. **Production container, audited Sites owner bootstrap, credential rotation, explicit edge-proxy transport checks, exact runtime grants, TLS/role/migration verification, signed provider-control evidence validation, and tenant-scoped REST/MCP two-phase signer activation intake are implemented. A real managed PostgreSQL instance and its live evidence remain deployment gates.**
...
13. Final Base mainnet readiness audit. **One read-only aggregate gate now authenticates the readiness, promotion, source-rehearsal, security-review, funded-signer, and signed ASCP release evidence records, separates twelve implementation capabilities from seventeen production blockers, and is required by the hardware broadcast wrapper. Its current and correct decision is `BLOCKED`.**
