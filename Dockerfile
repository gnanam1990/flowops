...
COPY --from=build /out/control-plane-api /out/flowops-admin /out/flowops-operator /out/ascp-leadership /out/ascp-seller-worker /out/ascp-event-recovery /out/ascp-verifier /out/ascp-keeper /out/ascp-bearer-worker /out/ascp-signer-runtime /out/postgres-readiness /flowops/
COPY deploy/control-plane/entrypoint.sh /flowops/entrypoint.sh
RUN chmod 0555 /flowops/control-plane-api /flowops/flowops-admin /flowops/flowops-operator /flowops/ascp-leadership /flowops/ascp-seller-worker /flowops/ascp-event-recovery /flowops/ascp-verifier /flowops/ascp-keeper /flowops/ascp-bearer-worker /flowops/ascp-signer-runtime /flowops/postgres-readiness /flowops/entrypoint.sh
