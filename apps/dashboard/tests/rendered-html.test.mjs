  ...
  const upstream = createServer(async (request, response) => {
    ...
    if (request.url === "/v1/dashboard/snapshot") {
	  ...
	  return json(200, {
		...
		ascp: { pendingApprovals: [{ approvalId: ascpApprovalId, reviewDigest: exactASCPReview }] },
	  });
    }
      ...
      response.end(JSON.stringify({
		...
		ascp: {
		  ...
		  pendingApprovals: [{
			...
			operationId: `0x${"5".repeat(64)}`, agentId: "agent_live", taskId: "task_ascp_live",
			category: "Verified data", reason: "HUMAN_APPROVAL_THRESHOLD", policyVersion: "policy_live_1",
