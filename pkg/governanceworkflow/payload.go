...
func CallEscrowAddVerifier(
	...
	return bind(CallEscrowDomain, chainID, contractAddress, selector("addVerifier(address,uint64,bytes32,bytes32)"),
		[]abi.Argument{{Type: addressType}, {Type: uint64Type}, {Type: uint64Type}, {Type: uint64Type}, {Type: uint64Type}},
		common.HexToAddress(key), activeEpoch, pendingEpoch, pendingActivatesAt, nextEpoch)
}

func CallEscrowRevokeVerifier(chainID uint64, contractAddress, key string, activeEpoch uint64) (common.Hash, error) {
	if !address(key) || activeEpoch == 0 {
		return common.Hash{}, ErrInvalidPayload
	}
	return bind(CallEscrowDomain, chainID, contractAddress, selector("revokeVerifier(address,bytes32,bytes32)"),
		[]abi.Argument{{Type: addressType}, {Type: uint64Type}, {Type: boolType}, {Type: boolType}},
		common.HexToAddress(key), activeEpoch, false, true)
}

func CallEscrowPause(chainID uint64, contractAddress string) (common.Hash, error) {
	return bind(CallEscrowDomain, chainID, contractAddress, selector("setEmergencyPause(bytes32,bytes32)"),
		[]abi.Argument{{Type: boolType}, {Type: boolType}}, false, true)
}

func SpendAuthorizer(chainID uint64, contractAddress, current string, currentEpoch uint64, next string) (common.Hash, error) {
	if !address(current) || !address(next) || currentEpoch == 0 {
		return common.Hash{}, ErrInvalidPayload
	}
	return bind(SpendModuleDomain, chainID, contractAddress, selector("setSpendAuthorizer(address,bytes32,bytes32)"),
...
func SpendAllowlist(chainID uint64, contractAddress, target, currentCodeHash, nextCodeHash string) (common.Hash, error) {
	if !address(target) || !hash(currentCodeHash, true) || !hash(nextCodeHash, true) || currentCodeHash == nextCodeHash {
		return common.Hash{}, ErrInvalidPayload
	}
	return bind(SpendModuleDomain, chainID, contractAddress, selector("setEscrowAllowlist(address,bytes32,bytes32,bytes32)"),
...
func SpendCaps(chainID uint64, contractAddress string, current, next Caps) (common.Hash, error) {
	...
	return bind(SpendModuleDomain, chainID, contractAddress,
...
func SpendPause(chainID uint64, contractAddress string, current, next bool) (common.Hash, error) {
	if current == next {
		return common.Hash{}, ErrInvalidPayload
	}
	return bind(SpendModuleDomain, chainID, contractAddress, selector("setEmergencyPause(bool,bytes32,bytes32)"),
		[]abi.Argument{{Type: boolType}, {Type: boolType}}, current, next)
}

func SpendInvalidateNonces(chainID uint64, contractAddress string, nonces []string) (common.Hash, error) {
	...
	return bind(SpendModuleDomain, chainID, contractAddress, selector("invalidateNonces(bytes32[],bytes32,bytes32)"),
		[]abi.Argument{{Type: bytes32ArrayType}}, values)
}

func DirectoryPublish(chainID uint64, contractAddress string, proposal DirectoryProposal) (common.Hash, error) {
	...
	return bind(ServiceDirectoryDomain, chainID, contractAddress, selector("approveVersion(uint64,bytes32)"),
...
func DirectoryCancel(chainID uint64, contractAddress string, versionID uint64, proposalHash string) (common.Hash, error) {
	if versionID == 0 || !hash(proposalHash, false) {
		return common.Hash{}, ErrInvalidPayload
	}
	return bind(ServiceDirectoryDomain, chainID, contractAddress,
...
func bind(domain string, chainID uint64, contractAddress string, functionSelector [4]byte, types []abi.Argument, values ...any) (common.Hash, error) {
	argumentsHash, err := arguments(types, values...)
	if err != nil {
		return common.Hash{}, err
	}
	return payload(domain, chainID, contractAddress, functionSelector, argumentsHash)
}

func payload(domain string, chainID uint64, contractAddress string, functionSelector [4]byte, argumentsHash common.Hash) (common.Hash, error) {
	if (chainID != 8453 && chainID != 84532) || !address(contractAddress) {
		return common.Hash{}, ErrInvalidPayload
	}
	return crypto.Keccak256Hash(
		crypto.Keccak256([]byte(domain)), uintWord(new(big.Int).SetUint64(chainID)),
		common.LeftPadBytes(common.HexToAddress(contractAddress).Bytes(), 32), common.RightPadBytes(functionSelector[:], 32),
		argumentsHash[:],
	), nil
}
