...
func readinessDB(t *testing.T, tls bool, overrides map[string]map[string]bool, columnExtras map[string][]string) (*sql.DB, sqlmock.Sqlmock) {
	...
	contracts := map[string]map[string]bool{
		...
		"ascp_financial_tombstones":          {"SELECT": true, "INSERT": true},
		"ascp_asset_health":                  {"SELECT": true},
		"ascp_capacity_counters":             {"SELECT": true},
		"ascp_capacity_admissions":           {"SELECT": true},
		...
		"ascp_workflow_receipt_ownership":    {"SELECT": true, "INSERT": true},
