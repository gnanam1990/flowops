...
type CompletionAuthorityVerifier interface {
	VerifyWorkflowCompletion(context.Context, Workflow, CompletionReceipt) error
}

type governanceActionGates []GovernanceActionGate

func (g governanceActionGates) ValidateGovernanceAction(bound governanceworkflow.BoundAction) error {
	for _, gate := range g {
		if err := gate.ValidateGovernanceAction(bound); err != nil {
			return err
		}
	}
	return nil
...
func WithGovernanceActionGate(gate GovernanceActionGate) Option {
	return func(service *Service) error {
		...
		if service.actionGate != nil {
			service.actionGate = governanceActionGates{service.actionGate, gate}
		} else {
			service.actionGate = gate
		}
		if verifier, ok := gate.(CompletionAuthorityVerifier); ok {
...
type Service struct {
	...
	authorityVerifier CompletionAuthorityVerifier
...
func (s *Service) Create(ctx context.Context, actor Actor, idempotencyKey string, request CreateRequest) (Workflow, error) {
	...
	if chainBacked {
		...
		chainAction, actionErr := chainActionForBound(bound)
		if actionErr != nil {
			return Workflow{}, ErrInvalidWorkflow
		}
		workflow.ChainAction = chainAction
	}
...
func (s *Service) ObserveAndComplete(ctx context.Context, organizationID, workflowID string) (Workflow, error) {
	...
	if !validReceipt(receipt) || workflow.WorkflowID != receipt.WorkflowID || workflow.PayloadHash != receipt.PayloadHash ||
		workflow.ChainAction != receipt.ChainAction ||
	...
	if s.authorityVerifier != nil {
		if err := s.authorityVerifier.VerifyWorkflowCompletion(ctx, workflow, receipt); err != nil {
			return Workflow{}, fmt.Errorf("%w: %w", ErrInvalidReceipt, err)
		}
	}
